  ...
  return (
    <span className={`relative inline-block ${className}`}>
      <span
        ...
        style={{ animation: "glitchA 3.2s infinite", clipPath: "inset(0 0 60% 0)" }}
        aria-hidden="true"
      >
        {children}
      </span>
      <span
        className="absolute inset-0 text-[#5DA9FF] opacity-60"
        style={{ animation: "glitchB 3.2s infinite", clipPath: "inset(60% 0 0 0)" }}
        aria-hidden="true"
      >
        {children}
      </span>
...
export default function CyberToss() {
  ...
  const openPopup = () => {
    playClick();
    setPick(null); setResult(null); setOutcome(null);
    setOpen(true);
  };
  ...
  const choose = (side) => {
    if (flipping) return;
    playTap();
    setPick(side); setResult(null); setOutcome(null);
  };
  ...
  const resetRound = () => { playTap(); setPick(null); setResult(null); setOutcome(null); };
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6 font-mono relative overflow-hidden">
      <style>{`
        ...
        @keyframes coinIdle { 0%,100%{transform:rotateY(0deg)} 50%{transform:rotateY(20deg)} }
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-white/40 text-sm mt-2">Opens a heads/tails call — pure chance, for fun</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/85 flex items-center justify-center p-5 z-50" onClick={closePopup}>
          ...
          >
            ...
            <div
              ...
              style={{
                background: "radial-gradient(circle at 50% 30%, #1a0505, #0a0202 75%)",
              }}
            >
              {/* scanline */}
              <div
                className="absolute left-0 right-0 h-16 pointer-events-none"
                style={{ background: "linear-gradient(rgba(255,59,59,0) 0%, rgba(255,59,59,0.08) 50%, rgba(255,59,59,0) 100%)", animation: "scan 3s linear infinite" }}
              />
              ...
              <p className="text-[#ff8a8a] text-[10px] tracking-[0.25em] uppercase mb-1">
                Coin Call Terminal
              </p>
              <h2 className="text-white text-xl font-bold mb-3">
                <GlitchText>[ {flipping ? "FLIPPING" : result ? "RESULT" : "READY"} ]</GlitchText>
              </h2>
              ...
              <div className="mb-3" style={{ perspective: "600px" }}>
                <div
                  ...
                  className="w-16 h-16 rounded-full flex items-center justify-center text-lg font-bold select-none"
                  style={{
                    background: "linear-gradient(135deg, #ff5c5c, #7a0000)",
                    color: "#fff",
                    transformStyle: "preserve-3d",
                    animation: flipping ? "coinFlip 1.5s cubic-bezier(0.45,0.05,0.55,0.95) forwards" : "coinIdle 3s ease-in-out infinite",
              ...
              {!result ? (
                ...
                >
                  <Zap size={13} />
                  {flipping ? "FLIPPING..." : "EXECUTE FLIP"}
                </button>
              ) : (
                <div className="w-full max-w-[220px]">
                  <p className="text-white text-sm font-bold capitalize mb-0.5">
                    {result} {outcome === "win" ? "— you called it" : "— missed"}
                  </p>
